          ...
          {sortedData.map((item) => (
            <li key={item.id} className="col-span-1 divide-y divide-gray-200 rounded-lg bg-cyan-800 shadow-sm">
                <div className="flex flex-col w-full items-center justify-between p-6">
                    <h3 className="m-0 text-center text-sm font-medium text-white">{item.name} ({item.id})</h3>
                    <h3 className="m-0 text-center text-sm font-medium text-white">${item.quote.USD.price.toFixed(2)}</h3>
                </div>
            </li>
          ))}
        </ul>
